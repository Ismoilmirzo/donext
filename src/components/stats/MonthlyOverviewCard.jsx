...
export default function MonthlyOverviewCard({ rows = [], trendPercent = 0 }) {
  return (
    <Card>
      <h3 className="text-base font-semibold text-slate-100">Monthly Focus Trend</h3>
      ...
      <p className={`mt-2 text-sm ${trendPercent >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
        Trend: {trendPercent >= 0 ? '+' : ''}
        {Math.round(trendPercent)}% vs last month
      </p>
    </Card>
  );
}
