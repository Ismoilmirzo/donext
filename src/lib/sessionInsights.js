...
export async function generateSessionInsights(summary, task, userId, t) {
  ...
  try {
    ...
    if (task.project_id) {
      queries.push(
        supabase.from('tasks').select('status').eq('project_id', task.project_id),
        supabase
          .from('focus_sessions')
          .select('date')
    ...
    const [weekResult, avgTaskResult, projectTaskResult, projectSessionResult] = results;
    ...
    if (sessionsCount > 1) {
      const avgData = avgTaskResult.data;
      if (avgData?.length >= 3) {
    ...
    if (task.project_id && projectTaskResult) {
      const projectTasks = projectTaskResult.data;
      if (projectTasks?.length) {
        ...
        // Pace estimate
        const projectSessions = projectSessionResult?.data;
        if (completed >= 2 && projectSessions?.length >= 2) {
